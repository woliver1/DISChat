...
public int getMarshalledSize()
{
   ...
   marshalSize = marshalSize + this.variableData.length ;
   // -WO
   return marshalSize;
}
...
public void unmarshal(DataInputStream dis)
{
    try 
    {
       ...
       // WO
       /*
       The problem is that the variableData field is set at 8 bytes - we 
       need to look at the field in the PDU and see how long the variableData
       really is. Then we also need to see if that length is on a 64bit boundary 
       as thats whats in the DIS pdu. We have to read in the whole field
       (including padding) or the next field will not be reading from the 
       correct place in the input buffer.
       */
       int lengthinbytes = (int)variableDatumLength/8; 
       byte[] newData = new byte[lengthinbytes];
       int unitsOfEight = lengthinbytes / 8;
       int remainder = lengthinbytes % 8;
       
       /* Read the buffer */
       for (int idx = 0; idx < newData.length; idx++)
       {    
           newData[idx] = dis.readByte();
       }
       
       /* Check to see if there is padding and read it */
        if (remainder != 0) {
            unitsOfEight++;
            byte[] padd = new byte[(unitsOfEight * 8 - lengthinbytes)];
            for (int idx = 0; idx < padd.length; idx++) {
                padd[idx] = dis.readByte();
            }
        }

       //System.out.println("In VariableDatum.unmarshall (159)");
       //System.out.println("Variable Datum ID is     " + Long.toString(variableDatumID));
       //System.out.println("Variable Datum length is " + Long.toString(variableDatumLength));
       //System.out.println("variableData length is   " + Long.toString(variableData.length));
       //System.out.println("lengthinbytes is         " + Integer.toString(lengthinbytes));
    ...
    { 
      //System.out.println("Exception here (167)...");
      System.out.println(e); 
    }
